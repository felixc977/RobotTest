...
class ExampleLibrary(object):
    ...
    def start_modcycle(self):
        print('start_modcycle -> enter ')
        str_cmd = 'C:\\work\\test_tools\\ModS_Cycling_Test_Utility_v1.7.1\\ModS_Cycling_Test_Utility.exe -q'
        ret = os.system(str_cmd)
        print('start_modcycle -> exit, ret=%s' % (ret))
        return ret

    def start_reboot(self):
        print('start_reboot -> enter ')
        str_cmd = 'shutdown /g'
        ret = os.system(str_cmd)
        print('start_reboot -> exit, ret=%s' % (ret))
        return ret
